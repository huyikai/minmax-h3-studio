...
清晰度按百万像素选择，计算方式沿用 ComfyUI 常用的约定：`1 MP = 1024²` 像素。Studio 会根据画幅比例计算宽高，并四舍五入到 `32` 的倍数。当前档位为：

`0.4 / 0.5 / 0.6 / 0.7 / 0.8 / 0.9 / 0.98 / 1.2 / 1.5 / 2.0 MP`

`0.98 MP` 接近 MiniMax H3 的原生画布面积。`1.2 / 1.5 / 2.0 MP` 会超过原生画布，通常更吃显存、生成更慢，显存不足时可能失败，画面细节也不保证同比提升。高档位应先确认本机 GPU 的显存余量；长视频还会按段重复产生对应的计算成本。

## 长视频工作机制

- 每个片段都是独立队列项，可以连续提交多个片段，不必等上一段生成结束。
- 片段按全局 FIFO 顺序排队；后一段必须等前一段成功后才会真正提交给 ComfyUI。
- 等待前段的片段会挂起，不会越过失败或中断的片段运行。
- 重写第 N 段前需要二次确认。确认后会丢弃第 N 段之后的输入、排队项和已成功片段，并将重写后的第 N 段排到队尾。
- 第 1 段可以选择步数 `16 / 20 / 25`，后续片段沿用同一设置。第 1 段提交后，整条链的画幅和清晰度锁定。
- 每段默认使用随机 Seed。片段衔接依靠上一段保存的 Motion Context 潜变量，不要求复用 Seed。
- 所有片段完成后才能定稿；定稿后可以撤销定稿，继续从最后一个成功片段接写。
